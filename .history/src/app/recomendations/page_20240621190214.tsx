"use client"
import React from 'react'
import Container from '../_components/Container'
import Card from '../_components/Card'
import styles from "../_styles/cards.module.css"
import { useRef } from 'react'

export interface RecomendationUI2 {
  id: number
  recomenda: string
}

const page = () => {
  const buttonRef = useRef(null)
  const moving = () => {
    const button = buttonRef.current
    const container = button.parentElement

    // Obtiene el ancho del contenedor y del botón
    const containerWidth = container.offsetWidth
    const buttonWidth = button.offsetWidth

    // Calcula la nueva posición del botón, asegurando que no salga del contenedor
    const maxOffset = containerWidth - buttonWidth
    const newOffset = Math.random() * maxOffset

    button.style.transform = `translateX(${newOffset}px)`
  }
  const recom: RecomendationUI2[] = [
    { id: 1, recomenda: 'Cuando sonries, el mundo entero se detiene, es tan triste que no lo veas' },
    { id: 2, recomenda: 'Si me preguntas que si te ves bien, diria que no hay nada que cambiaria, eres increible, tal como eres' },
    { id: 3, recomenda: 'Ya deja de preocuparte, recuerda que' }]
  return (
    <Container>
      <div className={styles.container}>
        <div className={styles.cards}>
          {recom.map((dat) => (
            <Card key={dat.id} recomenda={dat} />
          ))}
        </div>

      </div>
      <button onMouseEnter={() => moving()} className={`inline-block px-2 py-1 bg-sky-700 rounded-sm ${styles.animation_button}`} >Cargar mas</button>
    </Container>
  )
}

export default page